...
### 4. 测试脚本
所有测试脚本已统一放在 `test/` 目录下，直接使用 Node 运行：

```bash
# TikTok 提取测试
node test/test-tiktok.js

# 修复验证（携带上下文头部下载）
node test/test-fix.js

# 浏览器自动化下载测试
node test/test-browser-download.js

# 增强下载测试
node test/test-enhanced-download.js

# 常规下载测试
node test/test-download.js

# 新链接测试
node test/test-new-url.js

# 页面调试（抓取页面 HTML 与 Cookie）
node test/debug-page.js
```
...
- **会话与反爬**：同样需要携带页面生成时的 `Cookie` 与 `Referer`，并使用移动端 UA（在 `index.js:21-23` 的上下文配置），以提升直链可访问性。若无法获取到页面会话头部，则回退为通用 UA + 平台 `Referer`（例如 `https://www.douyin.com/`）。
- **解决策略**：
...
│   ├── test-tiktok.js
│   ├── test-fix.js
│   ├── test-browser-download.js
│   ├── test-enhanced-download.js
│   ├── test-download.js
│   ├── test-new-url.js
│   └── debug-page.js
├── videos/                # 下载视频的存储目录
